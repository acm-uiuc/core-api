...
import { getEpochTimestamp } from "../functions/general.js";
...
const ticketingPlugin: FastifyPluginAsync = async (fastify, _options) => {
  ...
  fastify.get<{
  ...
  }>("/details/:eventId", {}, async (request, reply) => {
    ...
    try {
      ...
      const parsedResponse = unmarshall(response.Item);
      if (
        parsedResponse["event_sales_active_utc"] > getEpochTimestamp() ||
        parsedResponse["event_sales_active_utc"] == -1
      ) {
        throw new NotFoundError({ endpointName: request.url });
      }
